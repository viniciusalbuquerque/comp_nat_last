	...
	private void movimentoIndividual() {
		for(Peixe peixe : this.peixes) {
			...
			if(peixe.isBetterFitness(auxPeixe.getFitness())) {
				peixe = auxPeixe;
				System.out.println("movimento individual executado");
			}	
...
//							Main.printPeixe(new Peixe(gradeFinal));
							...
							System.out.println("\n\nMelhor peixe");
							Main.printPeixe(this.melhorPeixe);
	...
	private void swapDisciplinasParaOMelhor(
		...
		if(disciplinasParaOMelhor != null && disciplinasParaOMelhor.size() > 0) {
			...
			for(Disciplina disciplina : disciplinasParaOMelhor) {
				...
				boolean addedOuHorarioLotado = false;
					...
					for(int j = 0; j < gradePeixe[0].length; j++) {
						...
						if(tempHorario != null) {
							...
							if(tempHorario.getDisciplinas().contains(disciplina)) {
								...
								addedOuHorarioLotado = true;
								
								// Checando limite de salas por horário
								if(horarioPeixe.getDisciplinas().size() >= Main.numeroSalasDisponiveis){
									break;
								}
	...
	private void movimentoColetivoDivergente() {
		for(Peixe peixe : this.peixes) {
			...
			if(peixe != this.piorPeixe) {
				
				int distanciaParaPiorPeixe = peixe.distanciaParaPiorPeixe(this.piorPeixe);
				int countDeslocamentoDisciplinas = 0;
				Horario[][] gradePeixe = peixe.getGrade();
				
				for(int i = 0; i < gradePeixe.length && countDeslocamentoDisciplinas < 2*distanciaParaPiorPeixe/3; i++) {
					
					for(int j = 0; j < gradePeixe[0].length; j++) {
						
						Horario horarioPeixe = gradePeixe[i][j];
						ArrayList<Disciplina> disciplinasParaOPior = new ArrayList<Disciplina>();
						
//						System.out.println("\nPeixe inicial:\n");
//						Main.printPeixe(peixe);
						
						if(horarioPeixe != null) {							
							disciplinasParaOPior = horarioPeixe.getDisciplinasParaOPior();							
						} 
						
						else {
							Horario horarioPiorPeixe = this.piorPeixe.getGrade()[i][j];
							
							if(horarioPiorPeixe != null) {
								disciplinasParaOPior = horarioPiorPeixe.getDisciplinas();	
							}
						}
						
						swapDisciplinasParaOPior(disciplinasParaOPior, gradePeixe, horarioPeixe);
						
						if(disciplinasParaOPior != null && disciplinasParaOPior.size() > 0) {
							countDeslocamentoDisciplinas++;	
						}
						...
						if(qtdDisciplinasInicial != qtdDisciplinasFinal){
							System.out.println("\n\nDisciplinas para o pior");
							for(Disciplina disciplina : disciplinasParaOPior){
								System.out.println(disciplina.getNome());
							}
...
//							Main.printPeixe(new Peixe(gradeFinal));
							...
							lancarExcecaoMudancaQtdDisciplinas(qtdDisciplinasInicial,
									qtdDisciplinasFinal, "movimento coletivo divergente");
						}
					}
				}
			}
	...
	private void swapDisciplinasParaOPior(
			ArrayList<Disciplina> disciplinasParaOPior, Horario[][] gradePeixe,
					...
					for(int j = 0; j < gradePeixe[0].length; j++) {
						...
						if(tempHorario != null) {
							...
							if(!tempHorario.getDisciplinas().contains(disciplina)) {
								...
								// Checando limite de salas por horário
								if(tempHorario.getDisciplinas().size() >= Main.numeroSalasDisponiveis){
									break;
								}
								
								tempHorario.addDisciplina(disciplina);
								horarioPeixe.removeDisciplina(disciplina);
								break;
							}		
						}				
					}
...
//			Peixe peixe2 = peixe;
			if(disciplinas1.isEmpty()) {
				peixe2.getGrade()[linha1][coluna1] = new Horario();
			} else {
			...
			if(disciplinas2.isEmpty()) {
				peixe2.getGrade()[linha2][coluna2] = new Horario();
			} else {
			...
			if(peixe.isBetterFitness(peixe2.getFitness())) {
				System.out.println("Swap individual executado");
				peixe = peixe2;
			}
//		}
			return peixe;
